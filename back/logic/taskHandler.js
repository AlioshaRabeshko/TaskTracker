...
const CH = require('./coefficientHandler').CoeffitientHandler;
...
class TaskMock {
  ...
  static startTask(task_id) {
    return Date.now(); //save to session or cookies, mb return object "task_id":Date.now()
  }

  static pauseTask(task_id, uid, startTimestamp) {
    executionTime = TF.toTimeString(Date.now() - startTimestamp);
    TaskMock.updateTaskExecutionTime(task_id, uid, executionTime);
  }

  static endTask(task_id, uid, startTimestamp) {
    executionTime = TF.toTimeString(Date.now() - startTimestamp);
    TaskMock.updateTaskExecutionTime(task_id, uid, executionTime, 'done');
  }

  static updateTaskExecutionTime(
    task_id,
    uid,
    executionTime,
    state = 'pending'
